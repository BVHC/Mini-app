...
const TaskCard = ({ task }) => {
  ...
  return (
    <div className={`p-4 bg-white rounded-xl border ${task.status === 'done' ? 'border-green-100 bg-slate-50/50' : 'border-slate-100'} shadow-sm flex items-start gap-4 group transition-all hover:shadow-md hover:border-indigo-100`}>
      <button 
        onClick={() => {
      ...
      <div className="flex-1 min-w-0">
        <h3 className={`text-[15px] font-medium mb-1.5 truncate ${task.status === 'done' ? 'line-through text-slate-400' : 'text-slate-800'}`}>
          {task.title}
        </h3>
        
        <div className="flex items-center gap-3 text-xs">
          <span className={`px-2.5 py-0.5 rounded-full border ${PRIORITY_COLORS[task.priority]} capitalize font-medium`}>
            {task.priority}
          </span>
